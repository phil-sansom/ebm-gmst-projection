...
save(world.sims, file = "output/figure7-rcp85.RData")
...
width  = (210/25.4 - 2)/2
height = width/sqrt(2)
pdf("./fig/figure7-rcp85.pdf", width = width, height = height, pointsize = 8)

par(las = 1, mar = c(2.5,2.5,1,1)+0.1, mgp = c(1.5,0.5,0), tcl = -1/3,
    xaxs = "i", yaxs = "i")

## RCP4.5
plot(1850:2100, y1[[1]][1,], type = "n", ylim = c(-0.5,+6.5), 
     xlab = "Time", #ylab = "GMST anomaly (K)",
     ylab = expression(paste("GMST anomaly(", phantom()^degree ,"C)")), 
     yaxt = "n")

axis(2, seq(0,6,1))
axis(4, seq(0,6,1), labels = FALSE)
