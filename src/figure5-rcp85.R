...
y1 = list()
for (model in models)
  y1[[model]] = rbind(c(data$historical$tas[[model]],data$rcp85$tas[[model]]),
                      c(data$historical$toa[[model]],data$rcp85$toa[[model]]))
...
for (model in models) {
  ...
  load(paste0("./output/samples-rcp85-",model,"-sims.RData"))
  buffer[[model]] = sims
...
width  = 210/25.4 - 2
height = width/sqrt(2)
pdf("./fig/figure5-rcp85.pdf", width = width, height = height, pointsize = 8 )

par(las = 1, mar = c(2.5,2.5,1,1)+0.1, mgp = c(1.5,0.5,0), tcl = -1/3,
    xaxs = "i", yaxs = "i", mfrow = c(2,2))

## Temperature
plot(1850:2100, y1[[mm]][1,], type = "n", ylim = c(-1.0,+6.0), 
...
## Radiation
plot(1850:2100, y1[[mm]][2,], type = "n", ylim = c(-1.0,+4.0), 
...
pdf("./fig/figure5s-rcp85.pdf", width = 210/25.4, height = 148/25.4, pointsize = 8)
...
for (m in 1:length(models)) {
  ...
  ## Temperature
  plot(1850:2100, y1[[m]][1,], type = "n", ylim = c(-1.0,+8.0), 
       ylab = expression(paste("GMST anomaly (", phantom()^degree ,"C)")),
       yaxt = "n", main = models[m])
  
  axis(2, seq(-1.0,+8.0,+1.0))
  axis(4, seq(-1.0,+8.0,+1.0), labels = FALSE)
