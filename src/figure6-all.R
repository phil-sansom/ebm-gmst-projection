...
fco2.all = exp(zetas[,11])
k1.all   = exp(zetas[,5])
ecs.all = fco2.all/k1.all
round(mean(ecs.all), 1)
round(median(ecs.all), 1)
round(quantile(ecs.all, c(0.05,0.95)), 1)
density.all = density(ecs.all)
round(density.all$x[which.max(density.all$y)],1)


## Plot ECS distributions
width  = (210/25.4 - 2)/2
height = width/sqrt(2)
pdf("./fig/figure6-all.pdf", width = width, height = height, pointsize = 8 )
...
plot (density.world, col = 2, lwd = 2, 
      ...
      xlab = expression(paste("Equilibrium Climate Sensitivity (",
                              phantom()^degree ,"C)")))
lines(density.all, col = 4, lwd = 2)
axis(1, 0:8)
legend("topright", legend = c("13 models","24 models"), 
