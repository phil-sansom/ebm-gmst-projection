...
models = c("bcc-csm1-1","CanESM2","CCSM4","CNRM-CM5","FGOALS-s2","GFDL-ESM2G",
           ...
           "MRI-CGCM3","NorESM1-M")
data = lapply(data, function(x) list(tas = x$tas[models], toa = x$toa[models]))
models = names(data$abrupt4xCO2$tas)

## Model as observations
cv.model = models[as.numeric(Sys.getenv("PBS_ARRAYID"))]
models   = models[models != cv.model]
...
## Observations
z  = c(data$historical$tas[[cv.model]],data$rcp45$tas[[cv.model]][1:15])
...
## Save burn-in
save.image(paste0(output.dir,"/",base.name,"-",cv.model,"-",
...
for (b in 1:n.blocks) {
  ...
  file.name = paste0(output.dir, "/", base.name, "-", cv.model,"-",
